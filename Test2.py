...
#function returns assets' risk weight based on portfolio weights and covariance matrix
#this approach to the risk contribution is derived from the following:
#https://people.umass.edu/~kazemi/An%20Introduction%20to%20Risk%20Parity.pdf
#with this approach, the weights calculation is an iterative process
def risk_allocation(w, cov_matrix):
    ...
    #risk contribution of each asset based on its volatility and its covariance with respect to the other assets
...
for day in pf_returns.loc[initial_date:end_date].index:
    ...
    if day.month != current_month:
        ...
        #calculating difference between equal risk weights and current weights, in order...
        #to find the necessary trades for rebal 
...
#calculating portfolio's return and daily volatility
cumulative_portfolio_return=round((cumulative_portfolio_return-1)*100,2)
portfolio_daily_std=round(statistics.stdev(portfolio_return)*100,2)

print('\nPortfolio total holding period return: ' + str(cumulative_portfolio_return) + '%')

#number of trading sessions
nts=len(portfolio_return)

#adjust volatility for the three months holding period
portfolio_std=portfolio_daily_std*np.sqrt(nts)

print('\nPortfolio holding period volatility: ' + str(round(portfolio_std,2)) + '%')

#calculating VaR for daily return, with a 5% probability of abnormal losses
#this approach assumes normal return distribution
VaR=round((statistics.mean(portfolio_return)*100-norm.ppf(0.95)*portfolio_daily_std),2)
...
#print previously calculated sharpe
print('\nPortfolio\'s Sharpe Index: ' + str(sharpe))